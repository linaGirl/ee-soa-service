!function() {
    ...
    module.exports = new Class({
        ...
        , init: function(options) {
            ...
            // make the db availabe to the users
            if (options && options.orm && options.dbName) this.db = options.orm[options.dbName];
        ...
        , _sendRequest: function(request, callback, token) {
            ...
            else if (callback instanceof SOAResponse) {
            ...
            if (request instanceof SOARequest) {
                ...
                if (token) request.addAccessToken(type.object(token) ? token.toString() : token);
                ...
                if (token) request.accessToken = type.object(token) ? token.toString() : token;
        ...
        /**
        * set event log data from the permissions on the model
        * so that it is stored alongside the event log
        */
        , setEventLogData: function(request, model) {
            if (type.function(model.setEventLogData) && request.permissions) {
                const values = request.permissions.getAllValues();
                const dataSet = [];

                if (values) {
                    for (const map of values.keys()){
                        const obj = {};
