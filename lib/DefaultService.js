        ...
        module.exports = new Class({
            ...
            , useMiddleware: function(middleware) {
                if (middleware && typeof middleware.request === 'function') {
                    log('['+this.name+'] middleware «' + middleware + '» loaded!')
                    ...
                    , next = function(err) {
                        if(this._middleware.length > index) {
                            ...
                            this._request(req, function(err, data) {
                                if(err) {
                                    //return log(err); // TODO: do it with statusCodes, write on RES
                                    res.send(res.status.SERVICE_EXCEPTION, err);
                                    return;
                                }
            ...
            , _handleRequest: function(req, callback) {
                ...
                var res = new SOARequest.response.Response();
                    res.on('end', function(result) {
                        var err = null;
                        if(result.content && result.content.error) err = result.content;
                        
                        callback(err, result.content); //TODO: write status and check, give callback direct to on end, or write error on res
                    }.bind(this));
