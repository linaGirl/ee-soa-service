!function() {
    ...
    module.exports = new Class({
        ...
        , _handleResponse: function(err, result, request, callback, status, headers) {
            ...
            // pay the rate limit
            if (request.permission && type.function(request.permission.payRateLimit)) {

                if (headers['Rate-Limit-Cost'] && !isNaN(headers['Rate-Limit-Cost'])) {
                    request.permission.payRateLimit(parseInt(headers['Rate-Limit-Cost'], 10)).then((info) => {
                        if (info) {
                    ...
                    request.permission.getRateLimitInfo().then((info) => {
                        if (info) {
                            headers['Rate-Limit'] = `${info.capacity}/${info.interval}s`;
                            headers['Rate-Limit-Balance'] = info.left;
                        }
