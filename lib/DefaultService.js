    ...
    !function() {
        ...
        module.exports = new Class({
            ...
            , sendRequest: function(request, response) {
                if (this.accessToken && request.accessTokens) {
                    if (debugService) log.debug('[DefaultService] Added my service token ...');

                    this._setAccessToken(request);
                }
            ...
            /**
             * sets our request token on the request and all subrequests
             */
            , _setAccessToken: function(request) {
                if (this.accessToken && request.accessTokens) {
                    request.accessTokens.push(this.accessToken);

                    if (request.subRequests && request.subRequests.length) {
        				request.subRequests.forEach(this._setAccessToken.bind(this));
        			}
                }
            }
