!function() {
    ...
    module.exports = new Class({
        ...
        , request: function(request, response) {
            if (request.satisifiesVersion('2.x')) {
                // the enw and clean way to handle requests
                // controllers MUST return promises
                // the catch expression will ALWAYS return a server error
                // there is no longer a response object inside of controllers
                
            }
            else (request.satisifiesVersion('<2.x')) this.nextRequest(request, response);
            else response.send(res.statusCodes.SERVICE_EXCEPTION, new Error('The requested API version is not available!'));
        }
        ...
        , _handleResponse: function(err, result, req, callback, status, headers) {
            ...
            if(err) {
                if(!err.error) {
                    if (debugService) log.warn('['+[this.name, req.getCollection(), req.getActionName()].join('.')+'] Got error response: '), log(err);

                    err = {
                          error: 1
                        , status: status
                        , msg: '['+[this.name, req.getCollection(), req.getActionName()].join('.')+'] ' + err.message
                    };
                }
            }
            //==========================
