                    ...
                    if (subRequests.length && entities.length) {
                        this._handleSubRequests(subRequests, entities, function(err, cost) {
                            if (err) callback(err);
                                ...
                                // compute the cost of the request
                                if (entities && entities.length) cost += entities.length*Object.keys(entities[0]).length;

                                callback(err, entities, null, {'Rate-Limit-Cost': cost});
                            }
                        }.bind(this));
                    }
                    else {
                        ...
                        callback(null, entities, null, {'Rate-Limit-Cost': cost);
