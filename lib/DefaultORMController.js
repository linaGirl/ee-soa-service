(function() {
    ...
    const ORMFilter         = require('./ORMFilter')
    ...
    module.exports = new Class({
        ...
        , list: function(queryData, callback, baseQuery, interceptor) {
            ...
            let limit  = type.number(range.to) ? (range.to - offset) : 10;

            // only users with special capabilities may
            // request more than 100 rows in one request
            if (limit > 100 && (!request.permissions || !request.permissions.hasCapability('big-query'))) {
