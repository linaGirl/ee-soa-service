(function() {
    ...
    module.exports = new Class({
            ...
            this.root[this.specs.hasMany[collection].table.name](['*']).get(this.table, idFilter).find(function(err, mappings) {
                ...
                if(mappings)
                {
                    mappings.forEach(function(mapping) {
                        ...
                        var subEntityId = mapping[
                               this.specs.hasMany[collection].table[collection]
                            || this.specs.hasMany[collection].table[this.specs.hasMany[collection].modelName]
                            || this.specs.hasMany[collection].table.theirKey
                        ];
        ...
        , applyRowRestrictions: function(request, query) {
             // row restrictions
            if (request.permissions && request.permissions.hasRowRestrictions() && query.setRestrictions) {
                ...
                for (const key of values) query.setRestrictionVariable(key, values.get(key));
            }
        }
        ...
        , describe: function(request, callback) {
            ...
            data.isLegacy       = true;
            data.actions        = ['list', 'listOne', 'update', 'createOrUpdate', 'delete', 'createRelation', 'updateRelation', 'deleteRelation'].filter(action => typeof this[action] === 'function');
            ...
            this.permissions.getPermissions(request.tokens, this.serviceName, this.name, null, request).then((permissions) => {
