        ...
        module.exports = new Class({
            ...
            , _getObjectFilters: function(filters) {
                ...
                Object.keys(filters).forEach(function(filterName) {
                    ...
                    // OBJECT FILTERS
                    if(type.array(filter)) {
                        //TODO: don't take first element, there could be more then 1!
                        var ormFilter = ORMFilter.get(filter[0]);
                        if(ormFilter.error) {
                            errors.push(ormFilter.msg);
                        }
                        else {
                            objectFilters[filterName] = ormFilter;
                        }
                    }
                }.bind(this));
            ...
            , _handleHasManySubRequest: function(entities, subRequest, callback) {
                ...
                var idFilter                    = {};
                idFilter[this.specs.primaryKey] = ORM.in(ids);
                var subMap                      = {};

                this.root[this.specs.hasMany[collection].table.name](['*']).get(this.table).find(function(err, mappings) {
                    if(err) return callback(err);
                    ...
                    if(mappings)
                    {
                        mappings.forEach(function(mapping) {
                            ...
                            subMap[subEntityId].push(map[entityId]);
