!function() {
    ...
    module.exports = new Class({
         ...
         * service consturctor
         */
        , init: function(options, entityList, serviceImplementationDirectory) {
            if (!options || !type.object(options))  throw new Error('The constructor of the DataService implementation requires an options object passed as parameter 0!');
            if (!type.object(options.orm))          throw new Error('The constructor of the DataService implementation requires an orm object passed inside the options object (parameter 0)!');
            if (!type.string(options.databaseName)) throw new Error('The constructor of the DataService implementation requires the «databaseName» string passed inside the options object (parameter 0)!');
            ...
            // if the service directory wasn't passed to the constructor we shouldnt load controllers from the fs 
            if (serviceImplementationDirectory) this._controllerDir = serviceImplementationDirectory.substr(0, serviceImplementationDirectory.lastIndexOf('/'))+'/controller';
        ...
        , _loadControllers: function(options, callback) {
            ...
            fs.readdir(this._controllerDir || '', function(err, files) {
                if (err && this._controllerDir) callback(err);
                else {
                    if (files) {
